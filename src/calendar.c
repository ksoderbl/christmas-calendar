...
static char *calendar_name = "Christmas calendar";
...
static texture_t *texture[NUMTEXTURES+1] = { NULL, };
static char *texfilenames[NUMTEXTURES+1]  =
{
	DATADIR "/julmust.rgb",
	DATADIR "/numbers.rgb",
	DATADIR "/effects.rgb",
...
static void run_effect(int index)
{
	...
	hp = &hatches[index];
	printf("hatch number is %d\n", hp->h_effect);
	if (effect_to_run != -1) {
		printf("about to run effect with index %d\n", effect_to_run);
		return;
	}

	if (hp->h_open) {
		printf("hatch number is %d is already opened\n", hp->h_effect);
		return;
	}
	hp->h_open = 1;
	effect_to_run = index;
...
static void process_hit(int name)
{
	...
	printf("hit (name, index) = (%d, %d)\n", name, index);

	if (index < 0 || index >= 24) {
		printf("no interesting hit\n");
		return;
	}
...
#define MAX 6
static void calendar_motion_cb(int x, int y)
{
	int dx, dy;
...
static void calendar_idle_cb(void)
{
	...
	if (effect_to_run != -1) {
		...
		if (hp->h_angle < hp->h_maxangle) {
			...
			if (idle_time == 0)
				idle_time = (unsigned int)time(NULL);
			else {
...
static void calendar_mouse_cb(int button, int state, int x, int y)
{
	int hit;
...
static void calendar_keyboard_cb(unsigned char key, int x, int y)
{
	x=x; y=y;
	switch (key) {
	case 'l':
...
/*#endif*/
	for (i = 0; i < NUMTEXTURES; i++) {
		...
		if (texfilenames[i] == NULL)
			continue;
		
		texture[i] = tex_init(texfilenames[i]);
		tp = texture[i];
		if (!tp) {
			fprintf(stderr, "failed to load %s\n", texture[i]->tx_filename);
			return -1;
		}
...
int effect0_register(struct effect *ep)
{
	...
	ep->e_display  = calendar_display_cb;
        ...
        ep->e_special  = calendar_special_cb;
        ep->e_mouse    = calendar_mouse_cb;
        /*ep->e_motion   = calendar_motion_cb;*/
...
void calendar_restore_glut_callbacks(void)
{
	glutKeyboardFunc(calendar_keyboard_cb);
	glutSpecialFunc(calendar_special_cb);
	glutMouseFunc(calendar_mouse_cb);
}

...
static char *effect1_name = "Colored Cubes";
...
static void effect1_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case 's': /* status */
 ...
 */
void effect1_special_cb(int key, int x, int y)
{
	x=x;y=y;
	switch (key) {
...
int effect1_register(struct effect *ep)
{
	...
	ep->e_special  = effect1_special_cb;
	/*ep->e_mouse    = effect1_mouse_cb;*/
	/*ep->e_motion        = effect1_motion_cb;*/
	/*ep->e_passivemotion = effect1_passivemotion_cb;*/
	/*ep->e_idle          = effect1_idle_cb;*/

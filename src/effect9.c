...
static char *effect9_name = "Stars only";
...
static void effect9_display_cb(void)
{
	int i;
	char s[256];
...
static void effect9_reshape_cb(int w, int h)
{
...
static void effect9_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case 's': /* status */
 ...
 */
void effect9_special_cb(int key, int x, int y)
{
}

int effect9_init_cb(struct effect *ep)
{
	ep=ep;
...
void effect9_cleanup_cb(struct effect *ep)
{
	(void)ep;
...
int effect9_register(struct effect *ep)
{
	ep->e_init     = effect9_init_cb;
	ep->e_cleanup  = effect9_cleanup_cb;

	ep->e_display  = effect9_display_cb;
	ep->e_reshape  = effect9_reshape_cb;
	ep->e_keyboard = effect9_keyboard_cb;
	ep->e_special  = effect9_special_cb;
    /*ep->e_mouse    = effect9_mouse_cb;*/

	ep->e_name     = effect9_name;
	return 0;
}

...
static char *effect23_name = "Stars only";
...
static void effect23_display_cb(void)
{
	int i;
	char s[256];
...
static void effect23_reshape_cb(int w, int h)
{
...
static void effect23_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case 's': /* status */
 ...
 */
void effect23_special_cb(int key, int x, int y)
{
}
...
int effect23_register(struct effect *ep)
{
	...
	ep->e_special  = effect23_special_cb;
    /*ep->e_mouse    = effect23_mouse_cb;*/

	ep->e_name     = effect23_name;
	return 0;
}

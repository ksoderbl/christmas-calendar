...
static GLUquadricObj *qp = NULL; /* XXX */



static char *name = "Flag";
/* state variables */
...
static texture_t *textures[NUMTEXTURES+1] = { NULL, };
...
static void effect6_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case 's':
...
int effect6_init_cb(struct effect *ep)
{
	...
	qp = gluNewQuadric();
	if (!qp) {
		fprintf(stderr, "could not alloc quadric\n");
		return -1;
	}
...
int effect6_register(struct effect *ep)
{
	...
	ep->e_special  = effect6_special_cb;
	/*ep->e_mouse    = effect6_mouse_cb;*/
	/*ep->e_motion        = effect6_motion_cb;*/
	/*ep->e_passivemotion = effect6_passivemotion_cb;*/
	/*ep->e_idle          = effect6_idle_cb;*/

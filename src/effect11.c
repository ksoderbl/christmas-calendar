...
static char *effect11_name = "Stars only";
...
static void effect11_display_cb(void)
{
	int i;
	char s[256];
...
static void effect11_reshape_cb(int w, int h)
{
...
static void effect11_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case 's': /* status */
 ...
 */
void effect11_special_cb(int key, int x, int y)
{
}

int effect11_init_cb(struct effect *ep)
{
	ep=ep;
...
void effect11_cleanup_cb(struct effect *ep)
{
	(void)ep;
...
int effect11_register(struct effect *ep)
{
	ep->e_init     = effect11_init_cb;
	ep->e_cleanup  = effect11_cleanup_cb;

	ep->e_display  = effect11_display_cb;
	ep->e_reshape  = effect11_reshape_cb;
	ep->e_keyboard = effect11_keyboard_cb;
	ep->e_special  = effect11_special_cb;
    /*ep->e_mouse    = effect11_mouse_cb;*/

	ep->e_name     = effect11_name;
	return 0;
}

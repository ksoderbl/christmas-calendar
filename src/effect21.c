...
static char *effect21_name = "Stars only";
...
static void effect21_display_cb(void)
{
	int i;
	char s[256];
...
static void effect21_reshape_cb(int w, int h)
{
...
static void effect21_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case 's': /* status */
 ...
 */
void effect21_special_cb(int key, int x, int y)
{
}

int effect21_init_cb(struct effect *ep)
{
	ep=ep;
...
void effect21_cleanup_cb(struct effect *ep)
{
	(void)ep;
...
{
	ep->e_init     = effect21_init_cb;
	ep->e_cleanup  = effect21_cleanup_cb;

	ep->e_display  = effect21_display_cb;
	ep->e_reshape  = effect21_reshape_cb;
	ep->e_keyboard = effect21_keyboard_cb;
	ep->e_special  = effect21_special_cb;
    /*ep->e_mouse    = effect21_mouse_cb;*/

	ep->e_name     = effect21_name;
	return 0;
}

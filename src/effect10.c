...
static char *effect10_name = "Stars only";
...
static void effect10_display_cb(void)
{
	int i;
	char s[256];
...
static void effect10_reshape_cb(int w, int h)
{
...
static void effect10_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case 's': /* status */
 ...
 */
void effect10_special_cb(int key, int x, int y)
{
}
...
int effect10_register(struct effect *ep)
{
	...
	ep->e_special  = effect10_special_cb;
    /*ep->e_mouse    = effect10_mouse_cb;*/

	ep->e_name     = effect10_name;
	return 0;
}

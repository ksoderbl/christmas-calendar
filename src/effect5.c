...
static char *effect5_name = "Stars only";
...
static void effect5_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case 's': /* status */
 ...
 */
void effect5_special_cb(int key, int x, int y)
{
}

int effect5_init_cb(struct effect *ep)
{
	ep=ep;
...
void effect5_cleanup_cb(struct effect *ep)
{
	(void)ep;
...
{
	ep->e_init     = effect5_init_cb;
	ep->e_cleanup  = effect5_cleanup_cb;

	ep->e_display  = effect5_display_cb;
	ep->e_reshape  = effect5_reshape_cb;
	ep->e_keyboard = effect5_keyboard_cb;
	ep->e_special  = effect5_special_cb;
    /*ep->e_mouse    = effect5_mouse_cb;*/

...
static char *effect18_name = "Stars only";
...
static void effect18_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case 's': /* status */
 ...
 */
void effect18_special_cb(int key, int x, int y)
{
}

int effect18_init_cb(struct effect *ep)
{
	ep=ep;
...
void effect18_cleanup_cb(struct effect *ep)
{
	(void)ep;
...
{
	ep->e_init     = effect18_init_cb;
	ep->e_cleanup  = effect18_cleanup_cb;

	ep->e_display  = effect18_display_cb;
	ep->e_reshape  = effect18_reshape_cb;
	ep->e_keyboard = effect18_keyboard_cb;
	ep->e_special  = effect18_special_cb;
    /*ep->e_mouse    = effect18_mouse_cb;*/

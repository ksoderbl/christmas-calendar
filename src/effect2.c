...
static char *effect2_name = "\"Pulsar\"";
...
static void effect2_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case 'b':
 ...
 */
void effect2_special_cb(int key, int x, int y)
{
	x=x;y=y;
	switch (key) {
...
int effect2_register(struct effect *ep)
{
	...
	ep->e_reshape  = effect2_reshape_cb;
        ...
        ep->e_special  = effect2_special_cb;

...
static char *effect13_name = "Stars only";
...
static void effect13_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case 's': /* status */
 ...
 */
void effect13_special_cb(int key, int x, int y)
{
}

int effect13_init_cb(struct effect *ep)
{
	ep=ep;
...
void effect13_cleanup_cb(struct effect *ep)
{
	(void)ep;
...
{
	ep->e_init     = effect13_init_cb;
	ep->e_cleanup  = effect13_cleanup_cb;

	ep->e_display  = effect13_display_cb;
	ep->e_reshape  = effect13_reshape_cb;
	ep->e_keyboard = effect13_keyboard_cb;
	ep->e_special  = effect13_special_cb;
    /*ep->e_mouse    = effect13_mouse_cb;*/

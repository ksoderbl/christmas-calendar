...
static char *effect7_name = "Stars only";
...
static void effect7_display_cb(void)
{
	int i;
	char s[256];
...
static void effect7_reshape_cb(int w, int h)
{
...
static void effect7_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case 's': /* status */
 ...
 */
void effect7_special_cb(int key, int x, int y)
{
}

int effect7_init_cb(struct effect *ep)
{
	ep=ep;
...
void effect7_cleanup_cb(struct effect *ep)
{
	(void)ep;
...
int effect7_register(struct effect *ep)
{
	ep->e_init     = effect7_init_cb;
	ep->e_cleanup  = effect7_cleanup_cb;

	ep->e_display  = effect7_display_cb;
	ep->e_reshape  = effect7_reshape_cb;
	ep->e_keyboard = effect7_keyboard_cb;
	ep->e_special  = effect7_special_cb;
    /*ep->e_mouse    = effect7_mouse_cb;*/

	ep->e_name     = effect7_name;
	return 0;
}

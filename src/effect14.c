...
static char *effect14_name = "Euro Stars";
...
static void effect14_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case ' ':
...
static int effect14_init_cb(struct effect *ep)
{
	ep = ep;
	if ((init_textures()) < 0) {
		fprintf(stderr, "effect14: textures failed to init.\n");
		return -1;
	}
...
int effect14_register(struct effect *ep)
{
	...
	ep->e_special  = effect14_special_cb;
	ep->e_mouse    = NULL;

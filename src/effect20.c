...
static char *effect20_name = "Electricity";
...
#define MAX 6
static void effect20_motion_cb(int x, int y)
{
	int dx, dy;
...
#if 0
static void effect20_mouse_cb(int button, int state, int x, int y)
{

}
#endif

static void effect20_idle_cb(void)
{
	glutPostRedisplay();
}
...
static void effect20_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case 'l':
...
static int effect20_init_cb(struct effect *ep)
{
	...
	if ((e = init_quadrics()) < 0) {
		fprintf(stderr, "effect20: quadrics failed to init.\n");
		return  e;
	}

	if ((e = init_textures()) < 0) {
		fprintf(stderr, "effect20: textures failed to init.\n");
		return  e;
	}
...
int effect20_register(struct effect *ep)
{
	...
	ep->e_display  = effect20_display_cb;
        ...
        ep->e_special  = effect20_special_cb;
        /*ep->e_mouse    = effect20_mouse_cb;*/
        /*ep->e_motion        = effect20_motion_cb;*/
        ep->e_motion   = effect20_motion_cb;
        ep->e_idle     = effect20_idle_cb;

...
static char *effect12_name = "Stars only";
...
static void effect12_display_cb(void)
{
	int i;
	char s[256];
...
static void effect12_reshape_cb(int w, int h)
{
...
static void effect12_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case 's': /* status */
 ...
 */
void effect12_special_cb(int key, int x, int y)
{
}
...
int effect12_register(struct effect *ep)
{
	...
	ep->e_special  = effect12_special_cb;
    /*ep->e_mouse    = effect12_mouse_cb;*/

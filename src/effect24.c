...
static char *effect24_name = "Earth and Sun";
...
#define MAX 6
static void effect24_motion_cb(int x, int y)
{
	int dx, dy;
...
#if 0
static void effect24_mouse_cb(int button, int state, int x, int y)
{

}
#endif

static void effect24_idle_cb(void)
{
	glutPostRedisplay();
}
 ...
 */
void effect24_special_cb(int key, int x, int y)
{
	int i;
	x=x;y=y;
	switch (key) {
...
static void effect24_keyboard_cb(unsigned char key, int x, int y)
{
	x=x;y=y;
	switch (key) {
	case 'l':
...
static int effect24_init_cb(struct effect *ep)
{
	...
	if ((e = init_quadrics()) < 0) {
		fprintf(stderr, "effect24: quadrics failed to init.\n");
		return  e;
	}

	if ((e = init_textures()) < 0) {
		fprintf(stderr, "effect24: textures failed to init.\n");
		return  e;
	}
...
int effect24_register(struct effect *ep)
{
	...
	ep->e_display  = effect24_display_cb;
        ...
        ep->e_special  = effect24_special_cb;
        /*ep->e_mouse    = effect24_mouse_cb;*/
        /*ep->e_motion        = effect24_motion_cb;*/
        ep->e_motion   = effect24_motion_cb;
        ep->e_idle     = effect24_idle_cb;
